...
export class GraphQlDataRenderer implements DataRenderer {
  ...
  constructor(
    data: any,
    renderConfig: GraphQlRenderConfig,
    metadata: Metadata,
  ) {
    this.data = data;
  ...
  public fields(): string[] {
    ...
    if (metadata && metadata.fields) {
      return metadata.fields
        ...
        .filter(field => this.data.hasOwnProperty(field));
    }
    return [];
  }
  ...
  public renderField(field: string | number): JSX.Element {
    if (this.data.hasOwnProperty(field)) {
      if (this.data instanceof Array && typeof field === "number") {
        const fieldMetadata = getMetadata(
          this.data[field].__typename,
          this.metadata,
        );
        if (fieldMetadata != null) {
          return this.renderConfig
            .getComponent(field, fieldMetadata)
        ...
        if (metadataType != null && metadataType.fields != null) {
          const fieldMetadataMatches = metadataType.fields.filter(
            mdField => mdField.name === field,
          );
          if (fieldMetadataMatches.length > 0) {
            const fieldMetadata = convertToMetadataType(
