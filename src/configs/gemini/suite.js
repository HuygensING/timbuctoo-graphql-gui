...
function makeUrl(storyKind, selectedStory) {
  return `/storybook/iframe.html?selectedKind=${encodeURIComponent(
    storyKind,
  )}&selectedStory=${encodeURIComponent(selectedStory)}&dataId=0`;
}
    ...
    if (typeof mod !== "function") {
      throw new Error(
        folderName +
          "/" +
          file +
          " does not export a function as default export.",
      );
    }
